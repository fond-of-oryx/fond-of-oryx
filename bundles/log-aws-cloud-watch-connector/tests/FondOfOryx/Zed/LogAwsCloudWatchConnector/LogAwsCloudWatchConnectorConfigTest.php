...
namespace FondOfOryx\Zed\LogAwsCloudWatchConnector;
    ...
    public function testGetAwsLogGroupName(): void
    {
        ...
        $this->config->expects($callCount)
            ...
            ->willReturnCallback(static function ($key, $default = null) use ($self, $callCount) {
                ...
                switch ($count) {
                    case 1:
                        $self->assertSame(LogAwsCloudWatchConnectorConstants::AWS_LOG_GROUP_NAME_ZED, $key);
                        $self->assertSame(LogAwsCloudWatchConnectorConstants::AWS_LOG_GROUP_NAME_ZED_DEFAULT_VALUE, $default);

                        return LogAwsCloudWatchConnectorConstants::AWS_LOG_GROUP_NAME_ZED_DEFAULT_VALUE;
                }

                throw new Exception('Unexpected call count');
            });

        static::assertEquals(LogAwsCloudWatchConnectorConstants::AWS_LOG_GROUP_NAME_ZED_DEFAULT_VALUE, $this->config->getAwsLogGroupName());
    }
    ...
    public function testGetAwsLogStreamName(): void
    {
        ...
        $this->config->expects($callCount)
            ...
            ->willReturnCallback(static function ($key, $default = null) use ($self, $callCount) {
                ...
                switch ($count) {
                    case 1:
                        $self->assertSame(LogAwsCloudWatchConnectorConstants::AWS_LOG_STREAM_NAME_ZED, $key);
                        $self->assertSame(LogAwsCloudWatchConnectorConstants::AWS_LOG_STREAM_NAME_ZED_DEFAULT_VALUE, $default);

                        return LogAwsCloudWatchConnectorConstants::AWS_LOG_STREAM_NAME_ZED_DEFAULT_VALUE;
                }

                throw new Exception('Unexpected call count');
            });

        static::assertEquals(LogAwsCloudWatchConnectorConstants::AWS_LOG_STREAM_NAME_ZED_DEFAULT_VALUE, $this->config->getAwsLogStreamName());
    }
    ...
    public function testGetAwsLogLevel(): void
    {
        ...
        $this->config->expects($callCount)
            ...
            ->willReturnCallback(static function ($key, $default = null) use ($self, $callCount) {
                ...
                switch ($count) {
                    case 1:
                        $self->assertSame(LogAwsCloudWatchConnectorConstants::AWS_LOG_LEVEL_ZED, $key);
                        $self->assertSame(LogAwsCloudWatchConnectorConstants::AWS_LOG_LEVEL_ZED_DEFAULT_VALUE, $default);

                        return LogAwsCloudWatchConnectorConstants::AWS_LOG_LEVEL_ZED_DEFAULT_VALUE;
                }

                throw new Exception('Unexpected call count');
            });

        static::assertEquals(LogAwsCloudWatchConnectorConstants::AWS_LOG_LEVEL_ZED_DEFAULT_VALUE, $this->config->getAwsLogLevel());
    }
