...
class ReferenceErpOrderPageSearchQueryExpanderPlugin extends AbstractPlugin implements
    QueryExpanderPluginInterface
{
    ...
    {
        if (!isset($requestParameters[ErpOrderPageSearchConstants::PARAMETER_REFERENCE])) {
            return $searchQuery;
        }

        $reference = $requestParameters[ErpOrderPageSearchConstants::PARAMETER_REFERENCE];
        ...
        $this->getBoolQuery($searchQuery->getSearchQuery())
            ->addMust(new Terms(ErpOrderIndexMap::REFERENCE, $reference));
