...
class ExternalReferenceErpOrderPageSearchQueryExpanderPlugin extends AbstractPlugin implements
    QueryExpanderPluginInterface
{
    ...
    {
        if (!isset($requestParameters[ErpOrderPageSearchConstants::PARAMETER_EXTERNAL_REFERENCE])) {
            return $searchQuery;
        }

        $externalReferences = $requestParameters[ErpOrderPageSearchConstants::PARAMETER_EXTERNAL_REFERENCE];
        ...
        $this->getBoolQuery($searchQuery->getSearchQuery())
            ->addMust(new Terms(ErpOrderIndexMap::EXTERNAL_REFERENCE, $externalReferences));
